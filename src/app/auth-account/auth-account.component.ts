...
export class AuthAccountComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (tmpUserInfo.valid == 0) weui.alert('您的个人资料正在审核中，不能修改！');
    if (tmpUserInfo.valid == 1||tmpUserInfo.valid==3||tmpUserInfo.valid==5) {
      // weui.alert('您的个人资料审核通过，请及时补充提款资料！');
      this.router.navigate(['/auth/bank']);
    }
    if (tmpUserInfo.valid == 2) weui.alert('您的个人资料审核失败，请修改后重新提交！');
    if (tmpUserInfo.valid == 4) {
